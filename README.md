...
## 📌kill이란?
**프로세스를 종료하거나 신호를 보내기 위해 사용된다. 일반적으로 프로세스를 강제로 종료할 때 많이 사용되지만, 다양한 신호를 프로세스에 보낼 수 있다.**
- kill 명령어는 프로세스를 강제 종료시킨다.
- 좀비 프로세스나, 응답 없음, 비정상적으로 동작하는 프로세스들의 실행을 끝내게 해준다.
- 프로세스를 포어그라운드로 가져와서 ctrl + C 할 수도 있지만, PID를 확인해서 kill로 바로 종료 시킬 수 있다.
